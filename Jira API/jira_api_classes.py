...
apikey = 'cSXuw7jNlEGnc6nMmr53AAAF'
server = 'https://yourcompany.atlassian.net'
url = server + "/rest/api/latest/issue/%s"
...
class Issue():
  def __init__(self,issue_id):
    ...
    self.issue = jira.issue(issue_id)
    self.status_history_new()
  ...
  def priority(self):
    return self.issue.fields.priority

  def fix_versions(self):     # returns list of fix versions
    try:     return self.issue.fields.fixVersions[0].name
    except:  return "-"

  def affected_versions(self):     # returns list of affected versions
    try:     return self.issue.fields.versions[0].name
    except:  return "-"

  def affected_env(self):     # returns list of affected versions
    return self.issue.fields.customfield_10051.value
  ...
  def status_history_new(self):
    cng_log =  Changelog(self.id())
    cng_log.get_change_log()
    self.issue_status_history = cng_log.log_records

  def first_status_update(self):       # return dictionary with details of the last status update
    try:     return self.issue_status_history[0]['updated_time']
    except:  return "-"

  def last_status_update(self):       # return dictionary with details of the last status update
    try:     return self.issue_status_history[-1]['updated_time']
    except:  return "-"

  def print_status_history(self):
    format_str = "|%25s|%32s|%40s|%40s|%20s|"
    title = format_str  %  ("Updated by","Time","From","To","Time in Status")
    print "-" * 163;  print title;    print "-" * 163         # print header
    ...
    for record in self.issue_status_history:
       print  format_str % (record["updated_by"],record["updated_time"],record["from"],record["to"],
            self.get_time_period(
                self.issue_status_history[record_index-1]['updated_time'] if record_index>0 else self.created(),
                self.issue_status_history[record_index]['updated_time'])
                )
       record_index += 1


  def get_time_period(self,start_time,end_time):
      try:     return self._to_timeperiod(self._to_timestamp(start_time),self._to_timestamp(end_time))
      except:  return "-"

  def subtasks_(self):
    # return list of subtasks with their status
    stasks = jira.search_issues('parentEpic=%s' % self.id())       # not yet tested
    # TODO: return list of subtasks with statuses and last updated
    return stasks

  def epic_bugs(self):
    # return list of subtasks with their status
    epic_bugs = jira.search_issues('issuetype=Bug and parentEpic=%s' % self.id())
    # return list of subtasks with statuses and last updated
    return epic_bugs

  def linked_issues(self):
    # return list of linked issues with their status
    linked = self.issue.fields.issuelinks
    # TODO: return list of subtasks with statuses and last updated
    return linked
...
class Changelog():
   ...
   def _get_page_status_history(self,page_history_records):
       for history in page_history_records:
           for item in history['items']:
               # reporting assignee should be configurable with command-line option
   ...
   def _get_changelog_page(self):
      ...
      self._get_page_status_history(resp['values'])
      self.islast         = resp['isLast']
      if not self.islast:
          self.next_page_url = resp['nextPage']

   def get_change_log(self):
       self.page_url = server + "/rest/api/latest/issue/%s" % self.issue_id + "/changelog?maxResults=100&startAt=0"
       while not self.islast:
           self._get_changelog_page()
           self.page_url = self.next_page_url
...
if __name__ == '__main__':
    def print_all(id):
       ...
       print "Elapsed time: " +   b.get_time_period(b.created(),b.last_status_update())
       eBugs =  b.epic_bugs()
       print "Epic Bugs: ",
       for i in eBugs: print  "%s, " % i,
       print
...
# ====================================================
TODO:
1. BG-4583 does not return the full history. It should start from Samantha Open -> Epic in Progress March 2, 2020, 5:56 PM (DONE)
2. Get assignment history separately and along with status change history (DONE for along with status)
